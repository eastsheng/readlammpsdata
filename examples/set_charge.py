# correct charges from ligpargen
import numpy as np
import readlammpsdata as rld
def set_charge_from_key(keyfile):
    charge_list = []
    with open(keyfile,'r') as sc:
        for index, line in enumerate(sc,1):
            # print(line)
            if "charge" in line:
                # print(line)
                list_line = "".join(line).split()
                # print(list_line[2])
                charge_list.append(list_line[2])
        print(charge_list)

    return charge_list

def set_charge_from_lmp(lmpfile):
    charge_list = []
    Atoms = rld.read_data(lmp1, data_sub_str = "Atoms # full")
    Atoms = rld.str2array(Atoms)
    charge_list = Atoms[:,3]
    print(charge_list)
    return charge_list


if __name__ == '__main__':
    path = "./"
    # key1 = path+"tmp/UNK_0735D7.key"
    lmp1 = path+"UNK_0735D7.lmp"

    charge_file = path+"system.in.charges"
    # charge_list_1 = set_charge_from_key(key1)
    charge_list_1 = set_charge_from_lmp(lmp1)

    charge_sum1 = round(sum(np.array(charge_list_1).astype(float)),4)

    len1 = len(charge_list_1)

    with open(charge_file,'w') as cf:
        L1 = len1        
        for i in range(L1):
            cf.write("set type "+str(i+1)+" charge "+charge_list_1[i-1]+'\n')

    print("-------------charges-------------\n",
          lmp1,"(",1,"~",L1,"): sum of charges = ",charge_sum1,"\n")

    